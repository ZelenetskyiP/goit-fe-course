...
const getUsersByEyeColor = (arr, color) =>
  arr.filter(preset => preset.eyeColor === color);
...
const getUsersByGender = (arr, gender) =>
  arr.filter(preset => preset.gender === gender).map(preset => preset.name);

console.log(getUsersByGender(users, "male")); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]

const getInactiveUsers = arr => arr.filter(preset => !preset.isActive);

console.log(getInactiveUsers(users)); // [объект Moore Hensley, объект Ross Vazquez, объект Blackburn Dotson]

const getUserByEmail = (arr, email) =>
  arr.find(preset => preset.email === email);
...
const getUsersWithAge = (arr, min, max) =>
  arr.filter(preset => preset.age >= min && preset.age <= max);
...
const getUsersByFriend = (arr, name) =>
  arr
    .filter(preset => preset.friends.includes(name))
